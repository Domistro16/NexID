...
import { processProofFlowReceiptHashJobs } from "@/lib/services/proofFlowService";
...
export async function GET(request: Request) {
  try {
    assertCron(request);
    const result = await processProofFlowReceiptHashJobs(inputFromUrl(request));
...
export async function POST(request: Request) {
  try {
    ...
    const result = await processProofFlowReceiptHashJobs(body);
    return NextResponse.json({ ok: result.every((item) => item.ok), results: result });
  } catch (error) {
    return NextResponse.json(jsonError(error), { status: 400 });
  }
