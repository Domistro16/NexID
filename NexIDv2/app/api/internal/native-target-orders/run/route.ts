...
import { jsonError, nativeTargetOrderRunSchema } from "@/lib/server/validation";
import { runNativeTargetOrders } from "@/lib/services/nativeTargetOrderService";

export const dynamic = "force-dynamic";

function inputFromUrl(request: Request) {
  const url = new URL(request.url);
  return nativeTargetOrderRunSchema.parse({
...
function botInput(input: ReturnType<typeof nativeTargetOrderRunSchema.parse>) {
...
export async function GET(request: Request) {
  try {
    ...
    const result = await runNativeTargetOrders(botInput(input));
...
export async function POST(request: Request) {
  try {
    ...
    const input = nativeTargetOrderRunSchema.parse(body);
    const result = await runNativeTargetOrders(botInput(input));
    return NextResponse.json(result, { status: responseStatus(result, input.strict) });
  } catch (error) {
    return NextResponse.json(jsonError(error), { status: 400 });
