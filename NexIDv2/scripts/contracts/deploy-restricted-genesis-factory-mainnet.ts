...
async function main() {
  ...
  const feeRouterAddress = requiredAddress("NATIVE_FEE_ROUTER_ADDRESS");
  const stakeVaultAddress = requiredAddress("NATIVE_LAUNCH_STAKE_VAULT_ADDRESS");
  const resolutionManagerAddress = requiredAddress("NATIVE_RESOLUTION_MANAGER_ADDRESS");
  ...
  const oldFactoryAddress = optionalAddress("NATIVE_MARKET_FACTORY_ADDRESS");
