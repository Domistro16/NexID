...
function normalizeAcademyPath(pathname: string) {
  ...
  if (pathname.startsWith("/academy/")) {
    return pathname.slice("/academy".length) || "/";
  }
...
export default function AcademyShell({ children }: AcademyShellProps) {
  ...
  const inBrowse = canonicalPath === "/" || canonicalPath === "/campaigns";
  const inCampaign = canonicalPath.startsWith("/campaign/");
  const inLeaderboard = canonicalPath.startsWith("/leaderboard");
  const inDashboard = canonicalPath.startsWith("/dashboard");
  const inIdentity = canonicalPath.startsWith("/identity");
  const inEarnings = canonicalPath.startsWith("/earnings");
  const inFaq = canonicalPath.startsWith("/faq");
  const inInterview = canonicalPath.startsWith("/interview");
      ...
      <div className="app">
        <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
          ...
          <nav className="sb-nav">
            ...
            <Link href="/" className={`sb-btn ${inBrowse ? "on" : ""}`}>
            ...
            <Link href="/leaderboard" className={`sb-btn ${inLeaderboard ? "on" : ""}`}>
              <svg className="sb-ic" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
            ...
            <Link href="/dashboard" className={`sb-btn ${inDashboard ? "on" : ""}`}>
              <svg className="sb-ic" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
            ...
            <Link href="/identity" className={`sb-btn ${inIdentity ? "on" : ""}`}>
              <svg className="sb-ic" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
            ...
            <Link href="/earnings" className={`sb-btn ${inEarnings ? "on" : ""}`}>
              <svg className="sb-ic" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
        ...
        <div className="main">
          ...
          <nav className="bottom-nav">
            <div className="bottom-nav-inner">
              ...
              <Link href="/identity" className={`bn-item ${inIdentity ? "on" : ""}`}>
                <svg className="bn-ico" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                </svg>
                <span className="bn-lbl">Identity</span>
              </Link>
              <Link href="/leaderboard" className={`bn-item ${inLeaderboard ? "on" : ""}`}>
                <svg className="bn-ico" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
                </svg>
