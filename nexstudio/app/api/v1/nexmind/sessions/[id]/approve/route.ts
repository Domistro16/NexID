...
export async function POST(request: Request, context: Context) {
  ...
  if (proposal.kind === "production") {
    ...
    if (productionId) {
      ...
      const permitted = new Set(["DRAFT", "SOURCE_READY", "DIRECTION_READY", "PAID", "LIVE_SESSION_READY", "LIVE_SESSION_ACTIVE", "BRIEF_REVIEW"]);
      ...
      const status = new Set(["PAID", "LIVE_SESSION_READY", "LIVE_SESSION_ACTIVE", "BRIEF_REVIEW"]).has(current.status) ? "BRIEF_REVIEW" : "DIRECTION_READY";
