...
export async function POST(request: Request, context: Context) {
  ...
  if (!new Set(["PAID", "LIVE_SESSION_READY", "LIVE_SESSION_ACTIVE", "BRIEF_REVIEW"]).has(production.status)) return problem(auth.id, 409, "PRODUCTION_NOT_READY_FOR_NEXMIND", "Production is not ready", "Confirm payment before starting the live direction session.");
  ...
  const session = await prisma.$transaction(async (tx) => {
    ...
    if (production.status !== "LIVE_SESSION_ACTIVE") await tx.production.update({ where: { id }, data: { status: "LIVE_SESSION_ACTIVE" } });
