...
import { saveCompositionVersion, saveInfographicVersion } from "@/lib/production-artifacts";
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    if (production.kind === "VIDEO" && !new Set(["BRIEF_REVIEW", "REVISION_REQUESTED", "FAILED"]).has(production.status)) return problem(requestIdValue, 409, "BRIEF_APPROVAL_REQUIRED", "Video brief review required", "Finish the NexMind direction session and review its structured brief before production.");
    ...
    const submitted = await submitHyperFramesRender(
      {
        productionId: id,
        title: production.title,
        message: parsed.data.message,
