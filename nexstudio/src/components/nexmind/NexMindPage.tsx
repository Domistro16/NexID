...
export function NexMindPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void loadSession(id).catch((reason) => notify("Session unavailable", reason instanceof Error ? reason.message : "The NexMind session could not be loaded."));
    }, 0);
    return () => window.clearTimeout(timer);
  ...
  const complete = useCallback(async () => {
    if (!session) return;
    setBusy(true);
    try { await api(`/api/v1/nexmind/sessions/${session.id}/complete`, { method: "POST", body: "{}" }); setLive(false); await Promise.all([loadSession(session.id), loadHistory()]); }
  ...
  }, [api, loadHistory, loadSession, notify, session]);
