...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const productionReputationOnly = true;
  const signInDisabled = true;
        ...
        <section className="modal open" role="dialog" aria-modal="true" aria-label="Sign in">
          ...
          <div className="modal-body">
            ...
            <div className="sign-options">
              ...
              {process.env.NODE_ENV === "development" ? <button className="connection-choice" onClick={() => { document.cookie = "nex_session=nex-dev-bypass-token; path=/; max-age=2592000"; window.location.reload(); }}><i>DEV</i><span><b>Use local dev simulation</b><small>Bypass production gates only on the local development server.</small></span><Icon name="arrow" size="sm" /></button> : null}
            </div>
