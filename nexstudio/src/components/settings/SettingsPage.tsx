...
export function SettingsPage() {
  ...
  const [tab, setTab] = useState<Tab>(requestedTab === "connections" || requestedTab === "workspace" || requestedTab === "privacy" || requestedTab === "accessibility" ? requestedTab : "account");
  ...
  return <><header className="page-head"><div className="page-head-copy"><span className="page-kicker">Settings</span><h1>Control the experience around the work.</h1><p>Account, workspace, connection, privacy and accessibility choices remain explicit.</p></div></header><section className="utility-grid"><nav className="utility-nav">{tabs.map(([value, label]) => <button className={tab === value ? "active" : ""} key={value} onClick={() => setTab(value)}>{label}</button>)}</nav><div className="utility-content">
