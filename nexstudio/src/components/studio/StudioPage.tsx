...
const videoTypes = ["Product launch", "Explainer", "Product demo", "Feature release", "Announcement", "Social video", "Let NexMind recommend"];
...
export function StudioPage() {
  ...
  const persist = async () => {
    ...
    try {
      ...
      const directionRecord = mode === "video" ? {
        videoType, duration, destination, creativeDirection: direction, voice, primaryColour: colour,
        mustInclude, mustAvoid, sourceIds: [...resourceIds, ...uploadedSourceIds], aspectRatio: "16:9",
      } : {
      ...
      await api(`/api/v1/productions/${production.id}`, {
        method: "PATCH",
        headers: { "idempotency-key": crypto.randomUUID() },
        body: JSON.stringify({ direction: directionRecord, brief: { message: brief.trim(), source, approvedSourceRights: true } }),
      });
      await refresh();
  ...
  if (step === "review") {
    ...
    return <section className={`studio-review ${mode === "infographic" ? "infographic-review" : ""}`}>
      <header><button className="btn text" onClick={() => setStep("direction")}><Icon name="arrowleft" size="sm" /> Edit {mode === "video" ? "choices" : "task"}</button><span className="page-kicker">{mode === "video" ? "Direction gathered" : "Task ready"}</span><h1>{mode === "video" ? "One clear pass before you speak." : "Check what Studio will create."}</h1><p>{mode === "video" ? "NexMind has the source, format and first creative decisions. Check the summary before this data-backed creation is stored." : "NexMind has organised your description, source, size and assets into one production task. Correct anything that is wrong before you approve it."}</p></header>
      ...
      <section className={mode === "video" ? "studio-review-call" : "infographic-approval-card"}>
        {mode === "video" ? <><div className="review-presence-mini" aria-hidden="true"><i/><i/><i/></div><span>NexMind direction</span><h2>Store this direction and continue.</h2><p>The record remains editable. A live provider session and chain quote are opened from the saved creation, never simulated in this page.</p></> : <div><span>Ready to create</span><h2>Approve the task and save it to Studio.</h2><p>Studio will use the approved description and supplied material. There is no live conversation for infographic creation.</p></div>}
  ...
  if (step === "direction") {
    ...
    return <><header className="page-head compact-head"><div className="page-head-copy"><button className="btn text" onClick={() => setStep("entry")}><Icon name="arrowleft" size="sm" /> Change source</button><span className="page-kicker">{isVideo ? "Video direction" : "Infographic"}</span><h1>{isVideo ? "Check the source. Shape the first direction." : "Set the task."}</h1><p>{isVideo ? "Everything here is recommended. Change only what matters." : "Describe what the visual must communicate, choose the output size and add the material Studio should use."}</p></div></header>
  ...
  const isVideo = mode === "video";
  return <><header className="page-head studio-final-head"><div className="page-head-copy"><span className="page-kicker">Studio</span><h1>Create a video or make information visual.</h1><p>{isVideo ? "Begin with the finished piece. NexMind helps shape the message and direction before production starts." : "Describe the visual, add the source material and attach anything Studio should use."}</p></div></header>
  <section className="studio-source-card"><div className="studio-mode"><button className={isVideo ? "active" : ""} onClick={() => { setMode("video"); setDuration("30 seconds"); }}>Video <small>From 4-5 USDC</small></button><button className={!isVideo ? "active" : ""} onClick={() => { setMode("infographic"); setDuration("1920x1080"); }}>Infographic <small>0.10 USDC</small></button></div><label className="universal-source"><span>{isVideo ? "What are you working from?" : "Describe the infographic you want."}</span><textarea value={source} onChange={(event) => setSource(event.target.value)} placeholder={isVideo ? "Paste a website, announcement, tweet, social post, or describe the finished piece." : "Describe what the visual should communicate. You can also paste a website, report, announcement, post or raw data."}/></label><div className="source-actions"><button className="tool-button" onClick={() => document.querySelector<HTMLTextAreaElement>(".universal-source textarea")?.focus()}><Icon name="link" size="sm" /> Paste website</button><button className="tool-button" onClick={() => document.querySelector<HTMLTextAreaElement>(".universal-source textarea")?.focus()}><Icon name="message" size="sm" /> Paste text</button><button className="tool-button" onClick={() => fileInput.current?.click()}><Icon name="upload" size="sm" /> {isVideo ? "Upload files" : "Upload assets"}</button><button className="tool-button" onClick={() => setResourcesOpen((value) => !value)}><Icon name="vault" size="sm" /> Your resources</button><button className="tool-button" onClick={() => router.push("/marketplace?tab=my-work")}><Icon name="market" size="sm" /> Marketplace work</button></div><input ref={fileInput} type="file" multiple hidden accept="image/png,image/jpeg,image/webp,image/gif,video/mp4,video/webm,video/quicktime,.pdf,.csv,.txt,.docx,.pptx,.xlsx" onChange={(event) => setFiles(Array.from(event.target.files || []))}/>{files.length ? <div className="infographic-upload-summary"><b>{files.length} asset{files.length === 1 ? "" : "s"} selected</b><span>{files.map((file) => file.name).join(" · ")}</span></div> : null}{resourcesOpen ? <div className="resource-picker-list">{data.authenticated && data.sources.length ? data.sources.map((item) => <label key={item.id} className="consent-item"><input type="checkbox" checked={resourceIds.includes(item.id)} onChange={() => setResourceIds((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : [...ids, item.id])}/><span><b>{item.name || item.originalUrl || "Untitled source"}</b><span>{item.kind} · {item.status}</span></span></label>) : <p>{data.authenticated ? "Your resources is empty." : "Connect a wallet to use saved resources."}</p>}</div> : null}<footer className="studio-source-footer"><span><b>{isVideo ? "5 USDC video production" : "0.10 USDC infographic"}</b><small>{isVideo ? "One direction preview is available before payment." : "NexMind organises the task in text. There is no live call."}</small></span><button className="btn primary" onClick={continueFromEntry}>{isVideo ? "Review source" : "Continue"} <Icon name="arrow" size="sm" /></button></footer></section><p className="entry-access-note">{isVideo ? "You can add the source and shape the direction before signing in or connecting a wallet." : "Add the description and source material first. Payment is requested only after you approve the task."}</p>
