...
export function StudioPage() {
  ...
  <section className="studio-source-card"><div className="studio-mode"><button className={isVideo ? "active" : ""} onClick={() => { setMode("video"); setDuration("30 seconds"); }}>Video <small>Chain-priced</small></button><button className={!isVideo ? "active" : ""} onClick={() => { setMode("infographic"); setDuration("1920x1080"); }}>Infographic <small>Chain-priced</small></button></div><label className="universal-source"><span>{isVideo ? "What are you working from?" : "Describe the infographic you want."}</span><textarea value={source} onChange={(event) => setSource(event.target.value)} placeholder={isVideo ? "Paste a website, announcement, social post, or describe the finished piece." : "Describe what the visual should communicate. You can also paste a website, report, announcement, post or raw data."}/></label><div className="source-actions"><button className="tool-button" onClick={() => document.querySelector<HTMLTextAreaElement>(".universal-source textarea")?.focus()}><Icon name="link" size="sm" /> Paste website</button><button className="tool-button" onClick={() => document.querySelector<HTMLTextAreaElement>(".universal-source textarea")?.focus()}><Icon name="message" size="sm" /> Paste text</button><button className="tool-button" onClick={() => fileInput.current?.click()}><Icon name="upload" size="sm" /> Upload files</button><button className="tool-button" onClick={() => setResourcesOpen((value) => !value)}><Icon name="vault" size="sm" /> Your resources</button></div><input ref={fileInput} type="file" multiple hidden accept="image/png,image/jpeg,image/webp,image/gif,video/mp4,video/webm,video/quicktime,.pdf,.csv,.txt,.docx,.pptx,.xlsx" onChange={(event) => setFiles(Array.from(event.target.files || []))}/>{files.length ? <div className="infographic-upload-summary"><b>{files.length} asset{files.length === 1 ? "" : "s"} selected</b><span>{files.map((file) => file.name).join(" · ")}</span></div> : null}{resourcesOpen ? <div className="resource-picker-list">{data.authenticated && data.sources.length ? data.sources.map((item) => <label key={item.id} className="consent-item"><input type="checkbox" checked={resourceIds.includes(item.id)} onChange={() => setResourceIds((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : [...ids, item.id])}/><span><b>{item.name || item.originalUrl || "Untitled source"}</b><span>{item.kind} · {item.status}</span></span></label>) : <p>{data.authenticated ? "Your resources is empty." : "Connect a wallet to use saved resources."}</p>}</div> : null}<footer className="studio-source-footer"><span><b>{isVideo ? "30-second video production" : "Finished infographic"}</b><small>{isVideo ? "The exact quote is read from the pricing contract after approval." : "NexMind organises the task in text. There is no simulated call."}</small></span><button className="btn primary" onClick={continueFromEntry}>{isVideo ? "Review source" : "Continue"} <Icon name="arrow" size="sm" /></button></footer></section><p className="entry-access-note">Add the description and source material first. Payment is requested only after you approve the task.</p>
